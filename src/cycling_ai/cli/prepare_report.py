...
def find_athlete_profile(session_path: Path, athlete_dir: Path | None = None) -> Path | None:
    """
...
def prepare_report(
    ...
    for session_path in session_files:
        ...
        if profile_path:
            ...
            logger.warning("  No athlete profile found (will use data from session)")
    ...
    generator_info = {"tool": "cycling-ai", "version": "0.1.0", "command": " ".join(sys.argv)}
...
Examples:
  ...
  cycling-ai prepare-report \\
    ...
    parser.add_argument("--session", type=str, help="Single session JSONL file")
    ...
    parser.add_argument("--no-validate", action="store_true", help="Skip validation against schema")
