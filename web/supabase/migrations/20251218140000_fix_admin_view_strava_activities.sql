-- Migration: Fix admin_user_view to count strava_activities
-- Created: 2025-12-18
-- Description: Count from strava_activities table instead of activities table
--
-- Issue: The view was counting from 'activities' table but user activities
-- are stored in 'strava_activities' table since the Strava sync implementation.

-- Must drop and recreate to change the count query
...
SELECT
  ...
  ap.timezone,
  ap.units_system,
  -- Fixed: Count from strava_activities instead of activities
  (SELECT COUNT(*) FROM public.strava_activities WHERE user_id = u.id) AS total_activities,
